// dayData.js is used to get the daily data of a particular mill ID within a period
...
router.get('/', async (req, res) => {
    try {
        ...
        // connect to the database name DayData
        const database = client.db('DayData');
        ...
        if (collections.length === 0) {
            ...
            return res.status(404).send('Day collection does not exist. The mill was just installed. Please wait.');
        }

        // access to the collection of the related piID
        const dayCollection = database.collection(PiID);
        ...
        const result = await dayCollection.find(query).sort({ timestamp: 1 }).toArray();
